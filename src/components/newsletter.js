    ...
    <div className={styles.formContainer}>
      ...
      <div className={styles.inputContainer}>
        <input
          ...
          ref={(node) => (email = node)}
          type='email'
          placeholder={translate({ id: 'newsletter.placeholder', message: 'Enter your email address', description: 'The placeholder within the input' })}
          onKeyDown={event => event.key === "Enter" ? submit() : null}
          required
          autoComplete="email"
        />
        <a className={styles.submit} rel="noopener" onClick={submit} type="submit">
      ...
      {status === 'sending' && <div className={`${styles.status} ${styles.sending}`}><Translate id='newsletter.sending' description='The text stating the email is being registered in the newsletter list'>Sending...</Translate></div>}
      {status === 'error' && <div className={`${styles.status} ${styles.error}`} dangerouslySetInnerHTML={{ __html: message }} />}
      {status === 'success' && <div className={`${styles.status} ${styles.success}`} dangerouslySetInnerHTML={{ __html: message }} />}
    </div>
  );
};
